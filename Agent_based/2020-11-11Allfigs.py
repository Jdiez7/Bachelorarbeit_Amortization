...
def main ():
    ...
    verbrauch = 0.1306               #kgCO_2/km
    e_verbrauch = 0.165             #kWh/km
    ...
    if which ==1:
        ...
        G
        t = data2[:,0]
        ax4.set_title('N = 1',size=sizetitle)            # Titel fuer Plots
        ax1.set_ylim(-5*round_sig(1.2*np.max(factor*G/1000)), round_sig(1.2*np.max(factor*G/1000)))                             # Achsenlimits y-Achse
        ax1.set_ylabel(r'G(t) in tons $CO_2$',size=size)   # Beschriftung y-Achse
        ax2.set_ylabel(r'G(t) in tons $CO_2$',size=size)   # Beschriftung y-Achse
        ax3.set_ylabel(r'G(t) in tons $CO_2$',size=size)   # Beschriftung y-Achse
        ax1.plot(t+2020, factor*G/1000)
        bar.update(100/3)
        """Plot 1,1"""
        ax4.plot(t+b,factor*data1[:,2])
        ax4.set_ylabel('$N_{tot}^{llg}(t)$',size=size)
        ax5.set_ylabel('$N_{tot}^{llg}(t)$',size=size)
        ax6.set_ylabel('$N_{tot}^{llg}(t)$',size=size)
        ...
        G = data1[:,1]-data2[:,1]
        t = data2[:,0]
        ax5.set_title('N = 100',size=sizetitle)            # Titel fuer Plots
        ...
        G = data1[:,1]-data2[:,1]
        t = data2[:,0]
        ax6.set_title('N = 10000',size=sizetitle)            # Titel fuer Plots
        ...
        ax1.set_xlabel(xlabel,size=size)                # Beschriftung x-Achse
        ax2.set_xlabel(xlabel,size=size)                # Beschriftung x-Achse
        ax3.set_xlabel(xlabel,size=size)                # Beschriftung x-Achse
        ax4.set_xlabel(xlabel,size=size)                # Beschriftung x-Achse
        ax5.set_xlabel(xlabel,size=size)                # Beschriftung x-Achse
        ax6.set_xlabel(xlabel,size=size)                # Beschriftung x-Achse
        ax1.set_xticks(xticks)                # Beschriftung x-Achse
        ax2.set_xticks(xticks)                # Beschriftung x-Achse
        ax3.set_xticks(xticks)                # Beschriftung x-Achse
        ax4.set_xticks(xticks)                # Beschriftung x-Achse
        ax5.set_xticks(xticks)                # Beschriftung x-Achse
        ax6.set_xticks(xticks)                # Beschriftung x-Achse
        bar.update(100)
        ax1.axhline(y=0, c='k', ls='--')
        ax2.axhline(y=0, c='k', ls='--')
        ax3.axhline(y=0, c='k', ls='--')
        ax1.annotate(r"$\Delta t_{a} = 5.5$ years", xy=(2025.5, 0), xytext=(2030, 0.1*10**9),arrowprops=dict(arrowstyle="->"), size = 13)
        ax3.annotate(r"$\Delta t_{a} = 9.9$ years", xy=(2029.92, 0), xytext=(2035, -1*10**8),arrowprops=dict(arrowstyle="->"), size = 13)
        ...
        ax1.set_xlim(2017,2050)
        ax2.set_xlim(2017,2050)
        ax3.set_xlim(2017,2050)
        ax4.set_xlim(2017,2050)
        ax5.set_xlim(2017,2050)
        ax6.set_xlim(2017,2050)
        ...
        G = data1[:,1]-data2[:,1]
        t = data2[:,0]
        ax1.set_title('Net total emission $G_{rsc}(t)$',size=titlesize)            # Titel fuer Plots
        ...
        ax1.set_ylabel(r'Net total $CO_2$ emissions in t',size=size)   # Beschriftung y-Achse
        ...
        ax2.set_title('Growth function $N^{tot}_{rsc}(t)$',size=titlesize)            # Titel fuer Plots
        ...
        ax2.set_ylabel(r'Total amount of EVs',size=size)   # Beschriftung y-Achse
        ...
        ax3.set_title(r'Growth rate $\nu_{rsc}(t)$',size=titlesize)            # Titel fuer Plots
        ax3.set_xlim(t[0]+b, t[-1]+b)                            # Achsenlimits x-Achse
        ax3.set_ylim(-0.2*np.max(grad), 1.2*np.max(grad))                             # Achsenlimits y-Achse
        ax3.set_ylabel(r'Growth rate in $\frac{1}{year}$',size=size)   # Beschriftung y-Achse
        ax3.set_xlabel(r'Time $t$ in years',size=size)                # Beschriftung x-Achse
        ax1.annotate(r"$\Delta t_{a} = 14,8$ years", xy=(2034.79, 0), xytext=(2034, 2*10**7),arrowprops=dict(arrowstyle="->"), size = 15)
        ...
        ax3.text(0.05, 0.95, 'c)', horizontalalignment='center', verticalalignment='center', transform=ax3.transAxes,size =13)
        ...
        # np.savetxt('1_DATA_neuausstoss.txt', np.array([swipe[:,0], swipe[:,1]]).T,fmt="%.3f", header="t   amortization")
        swipe = np.loadtxt("1_DATA_neuausstoss.txt")
        ax1.set_title('Net initial emission $a_\mathrm{i}$',size=titlesize)
        ax1.set_xlim(swipe[:,0][0]/1000,swipe[:,0][-1]/1000)                            # Achsenlimits x-Achse
        ...
        ax1.set_xlabel(r'$a_\mathrm{i}$ in tons $CO_2$',size=size)                # Beschriftung x-Achse
        ax1.plot(swipe[:,0]/1000,b+swipe[:,1])
        ax1.tick_params(axis='both', which='major', labelsize=labelsize)
        ...
        #     bar.update(i/M*100)
        # swipe = np.array(swipe)
        # np.savetxt('1_DATA_neutralyear.txt', np.array([swipe[:,0], swipe[:,1]]).T,fmt="%.3f", header="t   amortization")
        swipe = np.loadtxt("1_DATA_neutralyear.txt")
        ax2.set_title('CN power generation $t_\mathrm{CN}$',size=titlesize)
        ...
        ax2.set_xlabel(r'$t_\mathrm{CN}$ in years',size=size)                # Beschriftung x-Achse
        ...
        #     #plt.plot(t,G)
        # swipe = np.array(swipe)
        # np.savetxt('1_DATA_exponent.txt', np.array([swipe[:,0], swipe[:,1]]).T,fmt="%.3f", header="t   amortization")
        swipe = np.loadtxt("1_DATA_exponent.txt")
        bar.finish()
        ax3.set_title('Exponential growth factor $c_\mathrm{exp}^\mathrm{rsc}$',size=titlesize)
        ...
        ax3.set_xlabel(r'$c_\mathrm{exp}^\mathrm{rsc}$',size=size)                # Beschriftung x-Achse
        ...
        ax1.vlines(5.6, 2020, 2020+15.02, color='r', ls='--')
        ax1.vlines(4.5, 2020, 2020+13.3, color='k', ls='--')
        ax1.vlines(6.8, 2020, 2020+16.66, color='k', ls='--')
        ax2.vlines(2050, 2020, 2020+15, color='r', ls='--')
        ax3.vlines(0.428, 2020, 2020+15.02, color='r', ls='--')
        ...
        G = data1[:,1]-data2[:,1]
        t = data2[:,0]
        ax1.set_title('relative emission rate', size=titlesize)            # Titel fuer Plots
        ...
        ax1.set_ylabel(r'relative $CO_2$ emission rate',size=size)   # Beschriftung y-Achse
        ax1.set_xlabel(r'Time $t$ in years',size=size)                # Beschriftung x-Achse
        ...
        # np.savetxt('WB.txt', np.array([data1[:,0], data1[:,1],data2[:,1],data1[:,2]]).T,fmt="%.3f", header="t   amortization")
        lel = np.loadtxt("WB.txt")
        t =lel[:,0]
        WB = lel[:,1]
        base = lel[:,2]
        ges_e=lel[:,3]
        ...
        ax2.set_title('Net total emission $G_\mathrm{wbs}(t)$',size=titlesize)            # Titel fuer Plots
        ax2.set_xlim(t[0]+b, t[-1]+b)                            # Achsenlimits x-Achse
        ax2.set_ylim(-5*np.max(G/1000*factor), 1.2*np.max(G/1000*factor))
        #ax.set_ylim(-200000, 100000)                     # Achsenlimits y-Achse
        ax2.set_ylabel(r'Net total $CO_2$ emissions in tons $CO_2$',size=size)   # Beschriftung y-Achse
        ax2.set_xlabel(r'Time $t$ in years',size=size)                # Beschriftung x-Achse
        ax2.plot(t+b, G*factor/1000)
        print('The amortization time is {0}'.format(amor_time(t, G)))
        ax2.axhline(y=0, c='k', ls='--')
        ax2.annotate(r"$\Delta t_\mathrm{a} = 17.8$ years", xy=(2037.83, 0), xytext=(2037, 0.25*10**9),arrowprops=dict(arrowstyle="->"), size = 15)
        ax2.set_xticks(np.arange(2020,2071,25))

        # ges_e = data1[:,2]
        # ges_e
        # t = data2[:,0]
        ax1.plot(t+b, ges_e*factor)
        ax1.set_title('Growth function $N^\mathrm{tot}_\mathrm{wbs}(t)$',size=titlesize)            # Titel fuer Plots
        ax1.set_xlim(t[0]+b, t[-1]+b)                            # Achsenlimits x-Achse
        ax1.set_ylim(-0.2*np.max(ges_e*factor), 1.2*np.max(ges_e*factor))                             # Achsenlimits y-Achse
        ax1.set_ylabel(r'Total amount of EVs',size=size)   # Beschriftung y-Achse
        ax1.set_xlabel(r'Time $t$ in years',size=size)

        grad=np.gradient(ges_e*factor/step)

        ax3.plot(t+b,grad)
        ax3.plot(t+b, 136617/47700000 *0.428* N * np.exp(0.428*t)*factor, linestyle = ':', color = 'r')
        ax3.set_title(r'Growth rate $\nu_{wbs}(t)$',size=titlesize)            # Titel fuer Plots
        ax3.set_xlim(t[0]+b, t[-1]+b)                            # Achsenlimits x-Achse
        ax3.set_ylim(-0.2*np.max(grad), 1.2*np.max(grad))                             # Achsenlimits y-Achse
        ax3.set_ylabel(r'Growth rate in $\frac{1}{year}$',size=size)   # Beschriftung y-Achse
        ax3.set_xlabel(r'Time $t$ in years',size=size)
        ...
        G = data1[:,1]-data2[:,1]
        t = data2[:,0]
        ax1.set_title('relative emission')            # Titel fuer Plots
        ax1.set_xlim(t[0]+b, t[-1]+b)                            # Achsenlimits x-Achse
        # ax1.set_ylim(-1.2*np.max(G/data2[:,1]/1000*factor), 1.2*np.max(G/data2[:,1]/1000*factor))                             # Achsenlimits y-Achse
        ...
        # G=G1-G2
        # t = data2[:,0]
        # np.savetxt('Difference.txt', np.array([G,t]).T,fmt="%.3f", header="t   amortization")
        data = np.loadtxt('Difference.txt')
        G=data[:,0]
        t=data[:,1]
        ax1.set_title('Difference of emission',size=titlesize)            # Titel fuer Plots
        ax1.set_xlim(t[0]+b, t[-1]+b)                            # Achsenlimits x-Achse
        # ax1.set_ylim(-1.2*np.max(G/data2[:,1]/1000*factor), 1.2*np.max(G/data2[:,1]/1000*factor))                             # Achsenlimits y-Achse
        ax1.set_ylabel(r'$G_{rsc}(t)-G_{wbs}(t)$ in tons $CO_2$',size=size)   # Beschriftung y-Achse
        ax1.set_xlabel(r'Time $t$ in years',size=size)                # Beschriftung x-Achse
        #ax1.plot(t, G/1000*factor)
        ax1.plot(t+b,G/1000*factor)
        ax1.tick_params(axis='both', which='major', labelsize=12)
        ax1.set_xticks(np.arange(2020,2070,10))
        print('The amortization time is {0}'.format(amor_time(t, G)))
        plt.savefig("/Users/diez/X-Uni/2020_0SoSe/BA/Finalfigs/2difference.pdf")
        ...
        #     bar.update(i/M*100)
        # swipe = np.array(swipe)
        # np.savetxt('2_DATA_neuausstoss.txt', np.array([swipe[:,0], swipe[:,1]]).T,fmt="%.3f", header="t   amortization")
        swipe = np.loadtxt('2_DATA_neuausstoss.txt')
        ax1.set_title('Net initial emission $a_\mathrm{i}$',size=titlesize)
        ax1.set_xlim(swipe[:,0][0]/1000,swipe[:,0][-1]/1000)                            # Achsenlimits x-Achse
        ax1.set_ylim(0+b, b+20)                             # Achsenlimits y-Achse
        ax1.set_ylabel(r'year of systemic $CO_2$ amortization',size=size)   # Beschriftung y-Achse
        ax1.set_xlabel(r'$a_\mathrm{i}$ in t',size=size)                # Beschriftung x-Achse
        ax1.plot(swipe[:,0]/1000,b+swipe[:,1])
        bar.finish()
        print('#####################Fertig1###################')
        ...
        #     bar.update(i/M*100)
        # swipe = np.array(swipe)
        # np.savetxt('2_DATA_neutralyear.txt', np.array([swipe[:,0], swipe[:,1]]).T,fmt="%.3f", header="t   amortization")
        swipe = np.loadtxt('2_DATA_neutralyear.txt')
        ax2.set_title('CN power generation $t_\mathrm{CN}$',size=titlesize)
        ax2.set_xlim(2020,swipe[:,0][-1])                            # Achsenlimits x-Achse
        ax2.set_ylim(b+0, b+20)                             # Achsenlimits y-Achse
        ax2.set_ylabel(r'year of systemic $CO_2$-amortization',size=size)   # Beschriftung y-Achse
        ax2.set_xlabel(r'year of climate neutrality',size=size)                # Beschriftung x-Achse
        ax2.plot(swipe[:,0],b+swipe[:,1])
        ...
        #     #plt.plot(t,G)
        # swipe = np.array(swipe)
        # np.savetxt('2_DATA_exponent.txt', np.array([swipe[:,0], swipe[:,1]]).T,fmt="%.3f", header="t   amortization")
        swipe = np.loadtxt('2_DATA_exponent.txt')

        bar.finish()
        ax3.set_title('Exponential growth factor $c_\mathrm{exp}^\mathrm{wbs}$',size=titlesize)
        #ax3.set_xlim(swipe[:,0][0],swipe[:,0][-1])                            # Achsenlimits x-Achse
        ax3.set_ylim(0+b, b+20)                             # Achsenlimits y-Achse
        ax3.set_ylabel(r'year of systemic $CO_2$ amortization',size=size)   # Beschriftung y-Achse
        ax3.set_xlabel(r'$c_\mathrm{exp}^\mathrm{wbs}$',size=size)                # Beschriftung x-Achse
        ax3.plot(swipe[:,0],b+swipe[:,1])
        ax1.text(0.05, 0.95, 'a)', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
        ax2.text(0.05, 0.95, 'b)', horizontalalignment='center', verticalalignment='center', transform=ax2.transAxes)
        ax3.text(0.05, 0.95, 'c)', horizontalalignment='center', verticalalignment='center', transform=ax3.transAxes)
        yticks = np.arange(2020,2046,5)
        ax1.set_yticks(yticks)
        ax2.set_yticks(yticks)
        ax3.set_yticks(yticks)
        ax2.set_xticks(np.arange(2020,2071,25))
        ...
        ax2.tick_params(axis='both', which='major', labelsize=labelsize)
        ax3.tick_params(axis='both', which='major', labelsize=labelsize)
        ax1.vlines(5.6, 2020, 2020+17.85, color='r', ls='--')
        ax1.vlines(4.5, 2020, 2020+17.35, color='k', ls='--')
        ax1.vlines(6.8, 2020, 2020+18.53, color='k', ls='--')
        ax2.vlines(2050, 2020, 2020+17.94, color='r', ls='--')
        ax3.vlines(0.428, 2020, 2020+17.81, color='r', ls='--')

        print('#####################Fertig3###################')
        plt.savefig("/Users/diez/X-Uni/2020_0SoSe/BA/Finalfigs/26wbsparams.pdf")
